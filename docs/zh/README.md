...
| 改动 | 描述 |
|------|------|
| 🌐 **Web 管理面板** | 浏览器管理界面，访问 `/admin` — 查看可用模型、管理模型别名（创建/删除）、管理多个凭证配置（添加/删除/启用/禁用/验证）、查询配额用量，均通过 API Key 认证 |
...
| ️ **控制台编码修复** | 对非 UTF-8 控制台（如 Windows GBK）的启动横幅进行优雅降级 |
| 📝 **冻结模式支持** | 以 PyInstaller 打包运行时，正确配置日志和 uvicorn |
