...
| 🔧 **Web 端 API Key 配置** | 在 `/admin` 面板直接设置和更新 `PROXY_API_KEY`，服务端持久化保存，立即对所有客户端生效 |
| 🌐 **Web 管理面板** | 浏览器管理界面，访问 `/admin` — 查看可用模型、管理模型别名（创建/删除）、管理多个凭证配置（添加/删除/启用/禁用/验证）、查询配额用量 |
| 🔑 **多凭证管理** | 支持多个凭证配置轮询切换，支持按配置启用/禁用、验证、查询配额，通过 `CredentialManager` 实现 |
