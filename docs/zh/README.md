...
由 [@Jwadow](https://github.com/jwadow) 用 ❤️ 制作 | Fork 由 [@qingsheng-git](https://github.com/qingsheng-git) 维护
...
[Fork 改动](#-fork-改动) • [模型](#-可用模型) • [功能](#-功能特性) • [快速开始](#-快速开始) • [配置](#%EF%B8%8F-配置)
...
本项目是 [Jwadow/kiro-gateway](https://github.com/Jwadow/kiro-gateway) 的 Fork，新增了以下功能：

| 改动 | 描述 |
|------|------|
| 🚀 **零配置启动** | 无需 `.env` 文件即可启动服务，进入引导模式后通过 Web 管理面板配置凭证 |
| 🔧 **Web 端 API Key 配置** | 在 `/admin` 面板直接设置和更新 `PROXY_API_KEY`，服务端持久化保存，立即对所有客户端生效。智能密钥验证可优雅处理浏览器/服务端状态不一致的情况 |
| 🌐 **Web 管理面板** | 浏览器管理界面，访问 `/admin` — 查看可用模型、管理模型别名（创建/删除）、管理多个凭证配置（添加/删除/启用/禁用/验证）、查询配额用量、开关响应缓存 |
| 🔑 **多凭证管理** | 支持多个凭证配置轮询切换，支持按配置启用/禁用、验证、查询配额，启动时合并所有凭证的模型列表 |
| 💾 **响应缓存** | 内存 LRU 缓存，用于非流式 API 响应 — 相同请求直接返回缓存结果，减少 API 调用。可在管理面板中开关和清除缓存 |
| 🪟 **Windows 系统托盘模式** | 以后台服务方式运行网关，支持系统托盘图标、右键菜单启动/停止/重启、开机自启动、健康监控和错误通知 |
| 📦 **Windows 可执行文件打包** | 通过 PyInstaller 打包为独立 `.exe` 文件，无需安装 Python。单文件分发（约 50-80 MB），双击自动进入托盘模式 |
| 🧩 **新增模型** | 新增 DeepSeek-V3.2、MiniMax M2.1、Qwen3-Coder-Next 到支持的模型列表 |

> 上游功能（v2.3 及更早版本）已全部包含。上游更新日志请参见[原始仓库](https://github.com/Jwadow/kiro-gateway)。
...
| 📋 **扩展模型列表** | 包括版本化模型 |
| 🔐 **智能令牌管理** | 到期前自动刷新 |
| 💾 **响应缓存** | 内存 LRU 缓存，用于非流式请求，减少 API 调用 |
